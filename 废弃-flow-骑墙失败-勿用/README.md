> # ⛔️ 已退役 · 失败实验 · 请勿使用 ⛔️
> **这是 v1-hybrid,一个"骑墙"的失败尝试,不要拿去用、不要照着学。**
> 失败根因:①**骑墙**——背着"分解编排型"的重外化状态(`.flow/` 契约、per-slice 完成权),执行却是"陪伴型",两套架构各拿一半,好处都没吃全。②**建反顺序**——enforcement-first,让 skill 迁就脚本(正确应 skills-first、痛点驱动加脚本)。③**skill 认知不足**——最初没搞懂 skill 触发机制。
> 保留它只作**负结果记录**。真正决定架构的维度是"host 上下文可持续性"(高可持续→轻陪伴;低→分解编排),此实验没选定架构就动手了。新方向另起干净目录。
>
> ---

# flow(占位名 · 强制件骨架 v0)【已废弃】
